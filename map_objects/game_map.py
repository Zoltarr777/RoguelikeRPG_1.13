...
class GameMap:
	...
	def place_entities(self, room, entities, orc_tile, healing_potion_tile, scroll_tile, troll_tile, stairs_tile, 
		...
		monster_chances = {
			'goblin': from_dungeon_level([[50, 1], [50, 3], [30, 5], [0, 7]], self.dungeon_level),#80,1
		...
		for i in range(number_of_monsters):
			...
			if not any([entity for entity in entities if entity.x == x and entity.y == y]):
				...
				if monster_choice == 'goblin':
					fighter_component = Fighter(hp=4, defense=0, power=2, magic=0, magic_defense=0, xp=20, talismanhp=0, gold=randint(1, 3))
					...
					fighter_component = Fighter(hp=10, defense=0, power=3, magic=0, magic_defense=0, xp=50, talismanhp=0, gold=randint(2, 4))
					ai_component = BasicMonster()
					monster = Entity(x, y, slime_tile, libtcod.white, 'Slime', blocks=True,
						render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
				elif monster_choice == 'skeleton':
					fighter_component = Fighter(hp=10, defense=0, power=4, magic=0, magic_defense=0, xp=60, talismanhp=0, gold=randint(2, 5))
					ai_component = BasicMonster()
		...
		for i in range(number_of_items):
			...
			if not any([entity for entity in entities if entity.x == x and entity.y == y]):
				...
				if item_choice == 'healing_potion':
					heal1_amount = 20
					item_component = Item(use_function=heal, amount=heal1_amount, gold=3)
					item = Entity(x, y, healing_potion_tile, libtcod.white, "Health Potion" + " (+" + str(heal1_amount) + " HP)", render_order=RenderOrder.ITEM, item=item_component)
				elif item_choice == 'greater_healing_potion':
					heal2_amount = 40
					item_component = Item(use_function=heal, amount=heal2_amount)
					item = Entity(x, y, greater_healing_potion_tile, libtcod.white, "Greater Healing Potion" + " (+" + str(heal2_amount) + " HP)", render_order=RenderOrder.ITEM, item=item_component)
				...
				elif item_choice == 'slime_spawn':
					item_component = Item(use_function=slime_spawn)
					item = Entity(x, y, scroll_tile, libtcod.white, "Slime Spell", render_order=RenderOrder.ITEM, item=item_component)
				else:
