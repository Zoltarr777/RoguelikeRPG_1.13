...
from menus import main_menu, message_box, main_menu_help_menu, buy_menu
	...
	while not libtcod.console_is_window_closed():
		...
		if inventory_index is not None and previous_game_state != GameStates.PLAYER_DEAD and inventory_index < len(player.inventory.items):
			...
			if game_state == GameStates.SHOW_INVENTORY:
				...
				player_turn_results.extend(player.inventory.sell(item))
		...
		if buy_menu:
			...
			if buy_menu == 'item1':
				print("BOUGHT ITEM 1")
				player_turn_results.extend(player.inventory.buy(item))
			elif buy_menu == 'item2':
				print("BOUGHT ITEM 2")
				player_turn_results.extend(player.inventory.buy(item))
			elif buy_menu == 'item3':
		...
		if exit:
			if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY, GameStates.DROP_EQUIPMENT, GameStates.CHARACTER_SCREEN, GameStates.HELP_MENU, GameStates.SHOW_EQUIPMENT_INVENTORY, GameStates.SELL_MENU, GameStates.BUY_MENU):
				game_state = previous_game_state
